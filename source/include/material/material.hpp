...
public:
    ...
    // 取得散射率，wi和wo均以散射点为起点，beta为光线弹射的衰减
    virtual glm::vec3 evalBSDF(const glm::vec3 &wi, const glm::vec3 &wo) const { return {}; };
    // 在以法线为Y+的局部坐标系采样wo方向，其中wi和wo均以散射点为起点
    virtual glm::vec3 sampleScatteringDirection(const glm::vec3 &wi, const RNG &rng) const { return {}; };
    // 采样散射方向，并返回散射的衰减（beta）
    virtual glm::vec3 sampleBSDF(const glm::vec3 &wi, const RNG &rng, glm::vec3 &wo) const;
